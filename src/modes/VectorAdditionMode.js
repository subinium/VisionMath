export class VectorAdditionMode {
    ...
    _dragAt(fx, fy) {
        if (this.dragIndex === -1 && !this.dragOrigin) {
            const dO = Math.sqrt((fx - this.origin.x) ** 2 + (fy - this.origin.y) ** 2);
            if (dO < 0.04) { this.dragOrigin = true; return; }

            let minDist = 0.05;
            this.vectors.forEach((v, i) => {
                const d = Math.sqrt((fx - v.x) ** 2 + (fy - v.y) ** 2);
                if (d < minDist) { minDist = d; this.dragIndex = i; }
            });
        }

        if (this.dragOrigin) {
            this.origin.x = fx; this.origin.y = fy;
        } else if (this.dragIndex !== -1) {
            this.vectors[this.dragIndex].x = fx;
            this.vectors[this.dragIndex].y = fy;
        }
    }
    ...
    update(_results, { leftPinch, rightPinch }) {
        ...
        if (pinch) {
            if (this.dragIndex === -1 && !this.dragOrigin) {
                const dO = Math.sqrt((pinch.x - this.origin.x) ** 2 + (pinch.y - this.origin.y) ** 2);
                if (dO < 0.06) {
                    this.dragOrigin = true;
                } else {
                    let minDist = 0.06;
                    this.vectors.forEach((v, i) => {
                        const d = Math.sqrt((pinch.x - v.x) ** 2 + (pinch.y - v.y) ** 2);
                        if (d < minDist) { minDist = d; this.dragIndex = i; }
                    });
                }
            }
            if (this.dragOrigin) {
                this.origin.x = pinch.x; this.origin.y = pinch.y;
            } else if (this.dragIndex !== -1) {
    ...
    draw(ctx, w, h) {
        ...
        const ox = this.origin.x * w, oy = this.origin.y * h;
        ...
        const sumX = ox + v1x + v2x, sumY = oy + v1y + v2y;
        const diffX = ox + (v1x - v2x), diffY = oy + (v1y - v2y);

        const mag1 = Math.sqrt(v1x * v1x + v1y * v1y);
        const mag2 = Math.sqrt(v2x * v2x + v2y * v2y);
        const magR = Math.sqrt((v1x + v2x) ** 2 + (v1y + v2y) ** 2);
        const magD = Math.sqrt((v1x - v2x) ** 2 + (v1y - v2y) ** 2);
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(h1x, h1y); ctx.lineTo(sumX, sumY); ctx.lineTo(h2x, h2y);
        ctx.stroke();
        ctx.setLineDash([]);

        // Difference + sum vectors
        ...
        if (mag1 > 1) {
            ...
            const projX = ox + v1x * t, projY = oy + v1y * t;

            ctx.setLineDash([1, 4]);
            ctx.strokeStyle = 'rgba(56, 189, 248, 0.6)';
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(h2x, h2y); ctx.lineTo(projX, projY);
            ctx.stroke();
            ctx.setLineDash([]);
            ...
            ctx.lineWidth = 3;
            ctx.beginPath();
            ctx.moveTo(ox, oy); ctx.lineTo(projX, projY);
            ctx.stroke();
        ...
        if (mag1 > 1) {
            ...
            this.drawArrow(ctx, ox, oy, ox + ux * 40, oy + uy * 40, '#fb7185', 'v̂₁', null, { thin: true });
        }
        ...
        this.drawInfoPanel(ctx, w, { mag1, mag2, magR, magD, dot, cross, thetaDeg, v1x, v1y, v2x, v2y });
    }
    ...
    drawArrow(ctx, x1, y1, x2, y2, color, label, magnitude, opts = {}) {
        ...
        if (label) {
            const midX = (x1 + x2) / 2;
            const midY = (y1 + y2) / 2;
            const ox = -Math.sin(ang) * 12;
            const oy = Math.cos(ang) * 12;
            ctx.fillStyle = color;
    ...
    getControlsHTML() {
        return `
            <div class="control-item"><span class="icon">🤏</span><span class="text">Pinch a vector head</span></div>
            <div class="control-item"><span class="icon">⚪</span><span class="text">Drag origin to move axes</span></div>
            <div class="control-item"><span class="icon">📐</span><span class="text">Sum, diff, projection auto-compute</span></div>
        `;
    }
}
